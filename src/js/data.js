const data = [
	...
	},
	{
		...
		content: "<p>dear journal…</p>\n<p>it's the first day of May and the 7th (or 8th) day of Ramadan 1440 H and I've learned some basic data science about merging data from <code>pandas</code>.</p>\n<p><code>pandas</code> provides some methods that eases us to merge data: <code>join()</code>, <code>merge()</code>, and <code>concat()</code> (maybe more)…</p>\n<ul>\n<li><strong>inner join</strong> : returns rows from two tables that fulfill some conditions. (<code>pd.merge(left, right, on='key', how='inner')</code>)</li>\n<li><strong>left outer join</strong> : return all rows on the left table. (<code>pd.merge(left, right, on='key', how='left')</code>)</li>\n<li><strong>left outer join without intersection</strong> : return all rows on the left table without the center (intersection).</li>\n<li><strong>right outer join</strong> : return all rows on the right table. (<code>pd.merge(left, right, on='key', how='right')</code>)</li>\n<li><strong>right outer join without intersection</strong> : return all rows on the right table without the center (intersection).</li>\n<li><strong>full outer join</strong> : return all rows on both table. (<code>pd.merge(left, right, on='key', how='outer')</code>)</li>\n<li><strong>full outer join without intersection</strong> : return all rows on the both table without the center (intersection).[1]</li>\n</ul>\n<p><img src=\"https://www.datacourses.com/wp-content/uploads/2019/11/join-types-merge-names.jpg\" alt=\"table joins\" /></p>"
	}
];